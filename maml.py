...
class MAML:
    ...
    def conv3d_block(self, inp, cweight, bweight, reuse, scope, activation=tf.nn.relu, max_pool_pad='SAME',
                     residual=False):
    ...
    def optimize(self, weights, gradients, reuse=True, scope='optimizer'):
        ...
        output1 = self.conv3d_block(tf.transpose(tf.stack([gradients['conv1']]), perm=[0, 2, 3, 4, 1]),
                                    self.optimizer['conv1'], self.optimizer['b1'],
        ...
        output2 = self.conv3d_block(tf.transpose(tf.stack([gradients['conv2']]), perm=[0, 2, 3, 4, 1]),
                                    self.optimizer['conv2'], self.optimizer['b2'],
        ...
        output3 = self.conv3d_block(tf.transpose(tf.stack([gradients['conv3']]), perm=[0, 2, 3, 4, 1]),
                                    self.optimizer['conv3'], self.optimizer['b3'],
        ...
        output4 = self.conv3d_block(tf.transpose(tf.stack([gradients['conv4']]), perm=[0, 2, 3, 4, 1]),
                                    self.optimizer['conv4'], self.optimizer['b4'],
        ...
        output5 = tf.multiply(output5, self.optimizer['w5']) + self.optimizer['b5']
        ...
        with tf.variable_scope('model', reuse=None) as training_scope:
            ...
            def task_metalearn(inp, reuse=True):
                ...
                fast_weights = dict(zip(weights.keys(),
                                        updated_weights))  # [weights[key] - self.update_lr*gradients[key] for key in weights.keys()]
                ...
                for j in range(num_updates - 1):
                    ...
                    fast_weights = dict(zip(fast_weights.keys(),
                                            updated_weights))  # [fast_weights[key] - self.update_lr*gradients[key] for key in fast_weights.keys()]
                    output = self.forward(inputb, fast_weights, reuse=True)
                    task_outputbs.append(output)
                    task_lossesb.append(self.loss_func(output, labelb))
